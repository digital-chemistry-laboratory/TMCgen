...
score_norm_file = '.score_norm_torus.npy'
if os.path.exists(score_norm_file):
    score_norm_ = np.load(score_norm_file)
else:
    score_norm_ = score(
    sample(sigma[None].repeat(10000, 0).flatten()),
    sigma[None].repeat(10000, 0).flatten()).reshape(10000, -1)
    score_norm_ = (score_norm_ ** 2).mean(0)
    np.save(score_norm_file, score_norm_)
